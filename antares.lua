...
local function getSession()
	...
	local res = req({
		Url = "https://antares-awnn.onrender.com/session",
		Method = "POST",
...
local function redeemKey(session,key)
	local req = syn and syn.request or http_request or request
	local res = req({
		Url = "https://antares-awnn.onrender.com/redeem",
		Method = "POST",
		Headers = {["Content-Type"]="application/json"},
...
local function createMainUI()
	...
	-- ui frame of the main ui blud...
	local uiWidth, uiHeight = 400,420
	local mainUI = new("Frame",{Parent=gui, Size=UDim2.fromOffset(uiWidth,uiHeight), Position=UDim2.new(0,0,0,0), BackgroundColor3=Color3.fromRGB(0,0,0), BorderSizePixel=0, Visible=false})
	...
	ffBtn.MouseButton1Click:Connect(function()
		...
		local e
		do
			local f,g=pcall(function() return a:JSONDecode("{"..fastflags.."}") end)
			if f then e=g else return warn("JSON Decode Error:",g) end
		end
	...
	--  tab buttons thingies and ui thingy resizing incase i forget
	...
	for i,name in ipairs(tabNames) do
		...
		btn.MouseButton1Click:Connect(function()
			...
			-- idnicator slide thing so i wont forget
			...
			-- main ui resize thigny so i dont forget
	...
	-- icon cliccj fuckntion thing
	icon.MouseButton1Click:Connect(function()
		icon.Visible=false
		mainUI.Position=UDim2.new(0,icon.AbsolutePosition.X-uiWidth/2+iconSize/2,0,icon.AbsolutePosition.Y-uiHeight/2+iconSize/2)
		mainUI.Visible=true
	...
	closeBtn.MouseButton1Click:Connect(function()
		TweenService:Create(mainUI,TweenInfo.new(0.4),{Size=UDim2.fromOffset(0,0), BackgroundTransparency=1, Position=UDim2.new(0,icon.AbsolutePosition.X-uiWidth/2+iconSize/2,0,icon.AbsolutePosition.Y-uiHeight/2+iconSize/2)}):Play()
		delay(0.4,function() mainUI.Visible=false; icon.Visible=true end)
	end)
end

-- key validation section i think..?
